...
export default function PromptModal({
  ...
  const handleShare = async () => {
    ...
    try {
      const shareUrl = selectedPrompt.slug ? `${window.location.origin}/p/${selectedPrompt.slug}` : window.location.href
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto sm:ml-auto">
            {selectedPrompt && (
              <InteractionTracker promptId={selectedPrompt.id} action="copy">
                <Button
                  onClick={() => handleModalCopy(selectedPrompt.content, selectedPrompt.id)}
